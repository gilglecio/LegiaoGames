...
$SQL = mysql_query("SELECT * FROM `tb_posts` WHERE `postTitulo` LIKE '%$_SUBCONTROLLER%' LIMIT $inicio,$maximo") or die("<script type=\"text/javascript\">$(function(){alerta('O site não pode selecionar a tabela POSTES');});</script>");
...
<?php
$sqlResPaginatorPosts = mysql_query("SELECT * FROM `tb_posts` WHERE `postTitulo` LIKE '%$_SUBCONTROLLER%'");
